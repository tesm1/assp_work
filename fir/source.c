...
void fir_filter( int *restrict input_line, int *restrict output_line )
{
	...
	{
		result = mul(input_line[i], K0) + mul(input_line[i+1], K1) + mul(input_line[i+2], K2) + mul(input_line[i+3], K3);
...
void __attribute__((noinline)) copy_header( void )
{
	...
	{
		_TCEFU_FIFO_U8_STREAM_IN("stream_in", 0, input, status);
		_TCEFU_FIFO_U8_STREAM_OUT("stream_out", input);
	}
}
